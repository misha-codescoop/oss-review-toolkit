...
class BowerTest : StringSpec() {
    ...
    private fun patchExpectedResult(filename: String) =
            File(projectDir.parentFile, filename)
                    ...
                    .replaceFirst("<REPLACE_URL_PROCESSED>", normalizeVcsUrl(vcsUrl))
                    .replaceFirst("<REPLACE_REVISION>", vcsRevision)

    init {
        "Project dependencies are detected correctly" {
            val packageFile = File(projectDir, "bower.json")
            val expectedResult = patchExpectedResult("bower-expected-output.yml")
            val result = Bower.create().resolveDependencies(listOf(packageFile))[packageFile]
