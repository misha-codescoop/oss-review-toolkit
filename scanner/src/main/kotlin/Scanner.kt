...
abstract class Scanner(protected val config: ScannerConfiguration) {
     ...
     */
    abstract fun scan(packages: List<Package>, outputDirectory: File, downloadDirectory: File? = null, removeBinaryFiles: Boolean)
    ...
    fun scanDependenciesFile(dependenciesFile: File, outputDirectory: File, downloadDirectory: File? = null,
                             scopesToScan: Set<String> = emptySet(), removeBinaryFiles: Boolean): OrtResult {
        ...
        val results = scan(packagesToScan.map { it.pkg }, outputDirectory, downloadDirectory, removeBinaryFiles)
